...
### Fase 10 — Integración con Módulo Financiero (Sistema de Categorías Compartidas v1.2+)

**Objetivo:** Sincronizar ingresos y costos de vehículos con el módulo Financial de Aura Suite usando el nuevo sistema de categorías compartidas.

**NUEVO en Finanzas v1.2**: Categorías con soporte a múltiples módulos integradores.

**Flujo de configuración:**
1. Admin en **Finanzas → Categorías** crea dos categorías:
   - "Rental de Vehículos" con type `income` + marca ☑ **Vehículos**
   - "Mantenimiento de Vehículos" con type `expense` + marca ☑ **Vehículos**
   
2. Ambas se guardan con `integration_modules: ["vehicles"]`

3. En **Vehículos → Configuración**:
   - Selector "Categoría de ingreso (rental)" → solo muestra categorías con `vehicles` en integraciones
   - Selector "Categoría de egreso (mantenimiento)" → ídem
   - Checkbox "Crear automáticamente transacciones"

**Transacciones generadas:**
- Al cerrar una salida tipo `rental` con `total_amount > 0` → transacción de **ingreso** en categoría elegida
- Al cerrar una salida tipo `maintenance` con `maint_actual_cost > 0` → transacción de **egreso** en categoría elegida
- Los gastos detallados (`expenses_detail`) pueden crear transacciones separadas si se desea

**Ventajas:**
- Dashboard Financiero muestra automáticamente ingresos/gastos de flota
- Una sola categoría para todos los rentals, otra para todos los mantenimientos
- Auditoría: cada transacción vinculada a `source_module: "vehicles"` + `source_trip_id`

**Prerequisito:** El módulo Financial debe estar activo con v1.2+ del sistema de categorías.
