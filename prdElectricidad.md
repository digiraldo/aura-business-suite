...
### FASE 9 — Integración con Módulo Finanzas (opcional)

**Objetivo:** Al cerrar el período mensual, crear automáticamente una transacción en Finanzas por el costo del recibo eléctrico.

**Items:**

1. En `class-electric-reading-manager.php` o en `class-electric-alerts.php`:
   - Al calcular el consumo del mes si se detecta que cambia de período (día 1 del mes), verificar si el costo mensual supera un mínimo configurable
   - Si está activa la integración, crear transacción en `wp_aura_financial_transactions` con:
     - `category_id` = categoría de "Servicios Públicos" (configurable en settings del módulo)
     - `amount` = costo_total del mes
     - `description` = "Electricidad — {nombre_medidor} — {mes/año}"
     - `date` = último día del mes

2. En `admin/views/page-settings.php`:
   - Toggle "Registrar automáticamente recibos en Módulo Finanzas"
   - Selector de categoría financiera
   - Selector de usuario que aparece como "creado_por" en la transacción
...
| 9 | Integración con Finanzas | 🟢 Baja (opcional) | Fases 3, 4 |
